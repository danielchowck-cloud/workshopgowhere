        ...
        <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {popularIssues.map(({ model, issue }) => (
            ...
            >
              ...
              <p className="mt-3 line-clamp-3 text-xs leading-5 text-slate-600 dark:text-slate-400">
                {issue.diagnosticTip}
              </p>
              <div className="mt-4 rounded-xl bg-slate-50 p-3 text-xs dark:bg-slate-950">
                <div className="font-bold text-slate-900 dark:text-slate-100">Likely fix</div>
                <div className="mt-1 text-slate-600 dark:text-slate-400">{issue.likelyFix}</div>
                <div className="mt-2 flex items-center justify-between gap-3">
                  <span className="font-semibold text-emerald-700 dark:text-emerald-300">
                    Fair: ${issue.realFixPrice[0].toLocaleString()}–${issue.realFixPrice[1].toLocaleString()}
                  </span>
                  <span className="font-semibold text-rose-700 dark:text-rose-300">
                    Trap: ${issue.agentTrapPrice[0].toLocaleString()}+
                  </span>
                </div>
              </div>
              <div className="mt-3 flex items-center justify-between gap-3 text-xs">
                <span className="text-slate-500">Not 100% diagnosis — use this to ask better questions.</span>
        ...
        <div className="rounded-3xl bg-slate-950 p-6 text-white shadow-2xl shadow-slate-950/20 sm:p-8">
          <div className="grid gap-6 lg:grid-cols-[1fr_360px] lg:items-center">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black tracking-tight sm:text-3xl">Ready to sanity-check a quote?</h2>
              <p className="mt-3 text-sm leading-6 text-slate-300">
                Start with the symptom, understand the likely fix and fair price, then pick a workshop that can prove the diagnosis.
              </p>
            </div>
